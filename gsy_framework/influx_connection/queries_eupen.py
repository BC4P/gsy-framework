...
class DataQueryEupen(InfluxQuery):
    def __init__(self, influxConnection: InfluxConnection,
        ...
        qstring = f'SELECT mean("{power_column}") FROM "{tablename}" WHERE ("Location" = \'{location}\' AND "Key" = \'{key}\') AND time >= \'{start.to_datetime_string()}\' AND time <= \'{end.to_datetime_string()}\' GROUP BY time({interval}m), "Meter" fill(0)'    
        self.set(qstring)
